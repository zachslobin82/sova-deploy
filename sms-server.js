...
async function sendGhlSms(toPhone, message) {
  ...
  const contactRes = await fetch(
    `https://services.leadconnectorhq.com/contacts/?locationId=${CONFIG.ghlLocationId}&query=${encodeURIComponent((toPhone.replace(/\D/g,"").length===10?"+1":"+")+(toPhone.replace(/\D/g,"")))}`,
    {
      headers: {
